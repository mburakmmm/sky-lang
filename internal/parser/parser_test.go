...
const NAME = "SKY"`
	...
	tests := []string{"PI", "NAME"}

	for i, name := range tests {
		stmt := program.Statements[i]
		constStmt, ok := stmt.(*ast.ConstStatement)
		if !ok {
			t.Fatalf("stmt is not *ast.ConstStatement. got=%T", stmt)
		}
...
func TestFunctionStatement(t *testing.T) {
  ...
  return x + y
	...
	if !ok {
		t.Fatalf("stmt is not *ast.FunctionStatement. got=%T", program.Statements[0])
	}
...
func TestForStatement(t *testing.T) {
  ...
  print(item)
	...
	if stmt.Iterator.Value != "item" {
		t.Fatalf("stmt.Iterator.Value not 'item'. got=%s", stmt.Iterator.Value)
	}
}
...
func TestListLiteral(t *testing.T) {
	...
	if !ok {
		t.Fatalf("exp is not ast.ListLiteral. got=%T", stmt.Expression)
	}

	if len(list.Elements) != 3 {
		t.Fatalf("list.Elements has wrong length. got=%d", len(list.Elements))
	}
}
